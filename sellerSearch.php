<?php
//format compete. Needs more testing
...
echo("
    ...
    ");
    //see productSearch for explanation of margin-bottom
...
if(isset($_SESSION['username'])){
    ...
    if($_SESSION['username'] == 'Admin'){
        ...
        //search table. see productSearch for explanation
        $searchString = "%$search%";
        $sql = "SELECT 
            sellerId, 
            sales, 
            ...
            state_
            FROM sellers
            WHERE LOWER(sellerId) LIKE LOWER(:search)"; //case insensitive search, for my mom
        $stmt = $pdo->prepare($sql);
        ...
        if(count($result) > 0){ 
            //table header
            //no extra admin stuff this time since only admins can do this
            //why did I decide to only allow admins to do this? unknown
            echo("
            <table class='w3-table w3-bottombar'>
            <thead>
            <tr>
            <td class='w3-panel w3-border-bottom w3-border-right'> Seller </td>
            <td class = 'w3-panel w3-border-bottom w3-border-right'> Sales </td>
            <td class = 'w3-panel w3-border-bottom w3-border-right'> Country </td>
            <td class = 'w3-panel w3-border-bottom w3-border-right'> State </td>
            </tr>
            </thead>
            <tbody>
            ");
            ...
            foreach($result as $line){
                ...
                <td class='w3-panel w3-border-right'>".$line['sales']."</td>
                <td class='w3-panel w3-border-right'>".$line['country']."</td>
                <td class = 'w3-panel w3-border-right'>".$line['state_']."</td>
                </tr>
                ");
            }
            echo("
